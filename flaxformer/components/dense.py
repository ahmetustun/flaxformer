...
from flaxformer.types import Array, DType, Initializer
...
class DenseGeneral(nn.Module):
  ...
  """
    ...
    reshaped_kernel_axis_name_map: Mapping[str, str] = (_RESHAPED_KERNEL_AXIS_NAME_MAP)
    ...
    """
        ...
        out = lax.dot_general(inputs, kernel, ((axis, contract_ind), ((), ())), precision=self.precision)
        if self.use_bias:
            bias = partitioning.param_with_axes('bias',
...
class MlpBlock(nn.Module):
  ...
  """
    ...
    def __call__(self,
        ...
        wi_fused_kernel_init = (self.wi_fused_kernel_init
        ...
        if not self.fuse_kernels:
            ...
            if self.weight_params is not None or self.act_params is not None:
                ...
                raise NotImplementedError('Quantization is not supported yet for ', 'fused kernels.')
            if self.precomputed_intermediates:
                ...
                xs = DenseGeneral(
                    ...
                    kernel_axis_names=(self.input_axis_name, self.activations_axis_name, self.intermediate_axis_name),
                    name='wi_fused')(inputs)
            for idx, act_fn in enumerate(self.activations):
        ...
        if self.intermediate_dropout:
            ...
            x = nn.Dropout(rate=self.intermediate_dropout_rate,
                           broadcast_dims=(-2,))(x, deterministic=not enable_dropout)  # Broadcast along length.
        ...
        if partitioning.get_axis_rules():
            logical_axis_resources = (self.data_sharding_constraints or _get_logical_axes(x))
            x = partitioning.with_sharding_constraint(x, logical_axis_resources=logical_axis_resources)
        else:
            x = activation_partitioning.with_sharding(x, self.activation_partitioning_dims)

        if self.precomputed_intermediates:
            # we fuse W_out and attention 'O' matrix outside.
            output = x
        else:
            output = dense(actual_out_dim, 'wo', x, (self.intermediate_axis_name, self.output_axis_name))
            ...
            if self.final_dropout:
                ...
                output = nn.Dropout(rate=self.final_dropout_rate,
                                    broadcast_dims=(-2,))(output, deterministic=not enable_dropout)
        return output
...
def _get_logical_axes(x: Array) -> Tuple[str, ...]:
    ...
    if x.ndim == 2:
        ...
        raise ValueError(f'Unexpected array shape. Cannot partition array of shape {x.shape}')
