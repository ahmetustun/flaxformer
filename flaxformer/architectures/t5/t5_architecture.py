...
from flaxformer.components import (embedding, relative_position_biases, rich_attention_position_scores, transforms)
...
class MakeEncoderLayerFn(Protocol):
    ...
    def __call__(self, *, shared_relative_position_bias: Optional[nn.Module]) -> EncoderLayer:
...
class MakeDecoderLayerFn(Protocol):
    ...
    def __call__(self, *, shared_relative_position_bias: Optional[nn.Module]) -> DecoderLayer:
...
class EncoderLayer(nn.Module, param_remapping.ParameterRemappable):
  ...
  """
    ...
    def setup(self):
        if (self.relative_position_bias_factory is not None and self.shared_relative_position_bias is not None):
            raise ValueError('Please set at most one of relative_position_bias_factory and'
                             ' shared_relative_position_bias. (They can both be None however, e.g.'
                             ' for absolute position embeds.)')
        self.relpos_bias = (self.relative_position_bias_factory()
                            if self.relative_position_bias_factory is not None else self.shared_relative_position_bias)
    ...
    def get_bias(self, layer_input: Array) -> Optional[Array]:
        ...
        if isinstance(self.relpos_bias, relative_position_biases.RelativePositionBiases):
            encoder_bias = self.relpos_bias(layer_input.shape[-2], layer_input.shape[-2], bidirectional=True)
        elif isinstance(self.relpos_bias, rich_attention_position_scores.RichAttentionApi):
            encoder_bias = self.relpos_bias(layer_input, layer_input, bidirectional=True)
        else:
            raise TypeError(f'{type(self.relpos_bias)} is not a supported relative position '
    ...
    """
        ...
        layer_input = activation_partitioning.with_sharding_migration(layer_input,
                                                                      self.activation_partitioning_dims,
                                                                      logical_axis_names=('batch', 'length', 'embed'))
        if self.parallel:
            x = self.layer_norm(layer_input)
            x = activation_partitioning.with_sharding_migration(x,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))

            encoder_bias = self.get_bias(layer_input=x)

            y = (self.attention(x, x, inputs_embedding, encoder_mask, encoder_bias, enable_dropout=enable_dropout) +
                 self.mlp(x, inputs_embedding, enable_dropout=enable_dropout))
            y *= 2**-0.5
            ...
            x = self.attention(x, x, inputs_embedding, encoder_mask, encoder_bias, enable_dropout=enable_dropout)
            x = layer_input + self.post_attention_dropout(x, deterministic=not enable_dropout)
            x = activation_partitioning.with_sharding_migration(x,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))
            ...
            y = activation_partitioning.with_sharding_migration(y,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))
            ...
            # [batch, length, emb_dim] -> [batch, length, emb_dim]
            y = self.mlp(y, inputs_embedding, enable_dropout=enable_dropout)
...
class DecoderLayer(nn.Module, param_remapping.ParameterRemappable):
  ...
  """
    ...
    def get_bias(self, max_decode_length: Optional[int], decode: bool, layer_input: Array,
                 encoded: Array) -> Tuple[Optional[Array], Optional[Array]]:
        if not self.relpos_bias:
            return None, None

        if isinstance(self.relpos_bias, relative_position_biases.RelativeAttentionAPI):
            ...
            decoder_bias = self.relpos_bias(relpos_length, relpos_length, False, decode=decode)
            ...
            encoder_decoder_bias = self.relpos_bias(layer_input, encoded, bidirectional=False, is_cross_attention=True)
        else:
            raise TypeError(f'{type(self.relpos_bias)} is not a supported relative position '
                            f'bias factory.\nInstance value: {self.relpos_bias}')
        return decoder_bias, encoder_decoder_bias

    def _create_residuals_and_queries(self, layer_input: Array, x: Array, logit_mask: Array,
    ...
    """
        ...
        if prefill and prefill_lengths is None:
            ...
            prefill_lengths = jnp.sum(decoder_mask[:, 0, 0, :], axis=-1).astype(jnp.int32)

        if self.parallel:
            x = self.layer_norm(layer_input, decode=decode, prefill=prefill, prefill_lengths=prefill_lengths)
            x = activation_partitioning.with_sharding_migration(x,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))

            # Normally a no-op unless overridden by a subclass.
            layer_input_residual, x_queries, logit_mask_queries = (self._create_residuals_and_queries(
                layer_input, x, logit_mask, **kwargs))

            # Shared relative position embedding attention biases.
            decoder_bias, encoder_decoder_bias = self.get_bias(max_decode_length,
                                                               decode,
                                                               layer_input=x,
                                                               encoded=encoded)

            y = (self.self_attention(x_queries,
                                     x,
                                     inputs_embedding,
                                     decoder_mask,
                                     decoder_bias,
                                     ...
                                     decode=decode,
                                     prefill=prefill,
                                     prefill_lengths=prefill_lengths) + self.mlp(x,
                                                                                 inputs_embedding,
                                                                                 decode=decode,
                                                                                 prefill=prefill,
            ...
            if encoded is not None:
                y += self.encoder_decoder_attention(x,
                                                    encoded,
                                                    inputs_embedding,
            ...
            z = layer_input_residual + self.dropout(y, deterministic=not enable_dropout)
        else:
            # layer_input is derived from decoder_input_tokens.
            x = self.pre_self_attention_layer_norm(layer_input,
                                                   decode=decode,
                                                   prefill=prefill,
                                                   prefill_lengths=prefill_lengths)
            x = activation_partitioning.with_sharding_migration(x,
            ...
            x = self.self_attention(x_queries,
                                    x,
                                    inputs_embedding,
                                    decoder_mask,
                                    decoder_bias,
                                    ...
                                    prefill=prefill,
                                    prefill_lengths=prefill_lengths)
            x = layer_input_residual + self.post_self_attention_dropout(x, deterministic=not enable_dropout)
            x = activation_partitioning.with_sharding_migration(x,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))
            ...
            if encoded is None:
                ...
                y = self.encoder_decoder_attention(y,
                                                   encoded,
                                                   inputs_embedding,
                                                   encoder_decoder_mask,
                                                   encoder_decoder_bias,
                                                   enable_dropout=enable_dropout)
                y = x + self.post_cross_attention_dropout(y, deterministic=not enable_dropout)
                y = activation_partitioning.with_sharding_migration(y,
                                                                    self.activation_partitioning_dims,
                                                                    logical_axis_names=('batch', 'length', 'embed'))

            # MLP block.
            z = self.pre_mlp_layer_norm(y, decode=decode, prefill=prefill, prefill_lengths=prefill_lengths)
            z = activation_partitioning.with_sharding_migration(z,
                                                                self.activation_partitioning_dims,
                                                                logical_axis_names=('batch', 'length', 'embed'))
...
class Encoder(nn.Module, param_remapping.ParameterRemappable):
  ...
  """
    ...
    token_embedder_factory: Optional[Callable[[], embedding.Embedder[Array]]] = None
    shared_token_embedder: Optional[embedding.Embedder[Array]] = None
    position_embedder_factory: Optional[Callable[[], embedding.Embedder[Array]]] = None
    sow_intermediates: bool = False
    ...
    def setup(self):
        ...
        if (self.token_embedder_factory, self.shared_token_embedder).count(None) != 1:
            raise ValueError('Please set exactly one of token_embedder_factory or '
        ...
        if self.scan_layers and self.shared_relative_position_bias_factory:
            raise ValueError("Scanned layer mode doesn't support shared relative "
                             'position biases.')
        self.relpos_bias = (self.shared_relative_position_bias_factory()
        ...
        lyrf = maybe_remat(lyrf, self.layer_remat, self.scan_layers, static_argnums=(3,))
    ...
    def _construct_scanned_encoder(self, lyrf: Callable[[], nn.Module], num_layers: int) -> nn.Module:
        ...
        params_spec = self.scan_axis if initializing else transforms.ScanIn(self.scan_axis)
        cache_spec = 0
        intermediates_spec = 2  # Stacks intermediate layer outputs in dimension 2.
        scan_annotation = (self.spmd_annotations['encoder'] if self.spmd_annotations is not None else None)
        lyrf = transforms.factory_scan(
            ...
            length=num_layers,
            data_transform=transforms.inner_scan_spmd(scan_annotation, self.scan_axis),
    ...
    def embed_and_combine_inputs(self,
        ...
        embedded_inputs = self.input_dropout(embedded_inputs, deterministic=not enable_dropout)
    ...
    def encode_from_continuous_inputs(self,
        ...
        encoder_outputs = self.output_dropout(encoder_outputs, deterministic=not enable_dropout)
    ...
    """
        ...
        embedded_inputs = self.embed_and_combine_inputs(inputs,
                                                        inputs_positions=inputs_positions,
                                                        segment_ids=segment_ids,
                                                        enable_dropout=enable_dropout)
        logit_mask = jnp.expand_dims(jnp.array((inputs > 0), dtype=embedded_inputs.dtype), axis=-1)
        encoder_outputs = self.encode_from_continuous_inputs(embedded_inputs,
                                                             inputs_embedding,
...
class Decoder(nn.Module, param_remapping.ParameterRemappable):
  ...
  """
    ...
    def _setup_layer_sequence(self):
        lyrf = lambda: self.layer_factory(  # pylint: disable=g-long-lambda
            shared_relative_position_bias=self.relpos_bias)
        lyrf = maybe_remat(lyrf, self.layer_remat, self.scan_layers, static_argnums=(5, 6, 7, 8, 9))
        if not self.scan_layers:
            self.layers = [lyrf() for _ in range(self.num_layers)]
    ...
    def _construct_scanned_decoder(self,
                                   lyrf: Callable[[], nn.Module],
                                   num_layers: int,
        ...
        cache_spec = 0
        intermediates_spec = 2  # Stacks intermediate layer outputs in dimension 2.
        scan_annotation = (self.spmd_annotations['decoder'] if self.spmd_annotations is not None else None)
        lyrf = transforms.factory_scan(
            lyrf,
    ...
    def embed_and_combine_inputs(
        ...
        *,
        ...
        # TODO: Pass `deterministic=not enable_dropout`?
        embedded_inputs = self.embedder(segment_ids=segment_ids, decode=decode, **embedder_inputs)
    ...
    def decode_from_continuous_inputs(
        ...
        *,
        ...
        decoder_outputs = self.decoder(embedded_inputs,
                                       inputs_embedding,
                                       encoder_outputs,
        ...
        decoder_outputs = self.output_dropout(decoder_outputs, deterministic=not enable_dropout)
    ...
    """
        ...
        logit_mask = dense_attention.get_decoder_logit_mask(decoder_input_tokens, embedded_inputs.dtype)

        logits = self.decode_from_continuous_inputs(embedded_inputs,
                                                    inputs_embedding,
                                                    encoder_outputs,
                                                    decoder_positions=decoder_positions,
                                                    decoder_mask=decoder_mask,
...
class EncoderDecoder(nn.Module, param_remapping.ParameterRemappable):
  ...
  """
    ...
    def setup(self):
        self.token_embedder = (self.shared_token_embedder_factory() if self.shared_token_embedder_factory else None)
        ...
        encoder_factory_params = tuple(inspect.signature(self.encoder_factory).parameters.keys())
        ...
        decoder_factory_params = tuple(inspect.signature(self.decoder_factory).parameters.keys())
    ...
    def _make_padding_attention_mask(self, query_tokens: Array, key_tokens: Array) -> Array:
        return dense_attention.make_attention_mask(query_tokens > 0, key_tokens > 0, dtype=self.dtype)
    ...
    """
        ...
        encoder_mask = self._make_padding_attention_mask(encoder_input_tokens, encoder_input_tokens)
        # Add segmentation block-diagonal attention mask if using segmented data.
        if encoder_segment_ids is not None:
            encoder_mask = dense_attention.combine_masks(
                encoder_mask,
                dense_attention.make_attention_mask(encoder_segment_ids,
                                                    encoder_segment_ids,
                                                    jnp.equal,
                                                    dtype=self.dtype))
    ...
    """
        ...
        if decode:
            ...
            decoder_mask = None
            encoder_decoder_mask = self._make_padding_attention_mask(jnp.ones_like(decoder_target_tokens),
                                                                     encoder_input_tokens)
        else:
            decoder_mask = dense_attention.make_decoder_mask(decoder_target_tokens=decoder_target_tokens,
                                                             dtype=self.dtype,
                                                             decoder_segment_ids=decoder_segment_ids)
            encoder_decoder_mask = self._make_padding_attention_mask(decoder_target_tokens, encoder_input_tokens)

        # Add segmentation block-diagonal attention masks if using segmented data.
        if encoder_segment_ids is not None:
            if decode:
                raise ValueError('During decoding, packing should not be used but '
                                 '`encoder_segment_ids` was passed to `Transformer.decode`.')

            encoder_decoder_mask = dense_attention.combine_masks(
                encoder_decoder_mask,
                dense_attention.make_attention_mask(decoder_segment_ids,
                                                    encoder_segment_ids,
                                                    jnp.equal,
                                                    dtype=self.dtype))
        ...
        if decode:
            ...
            if 'decoder_attention_mask' in kwargs:
                ...
                if decoder_causal_attention is not None:
                    raise ValueError('Only one of `decoder_causal_attention` and '
